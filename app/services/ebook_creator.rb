...
module EbookCreator
  def self.kindle(current_user, user_directory, full_directory_path, ebook_file_name, folder)
    ...
    system("ebook-convert #{full_directory_path}.zip #{user_directory}/#{ebook_file_name}.#{book_format} --authors \"#{folder.name}\" --no-inline-toc --chapter 'page' --change-justification 'left' --page-breaks-before '/' #{mobi_file_type}")
  ...
  def self.epub(current_user, user_directory, full_directory_path, ebook_file_name, folder)
    system("zip -r #{full_directory_path}.zip #{full_directory_path}")
    system("ebook-convert #{full_directory_path}.zip #{user_directory}/#{ebook_file_name}.epub --authors \"#{folder.name}\" --chapter 'page' --change-justification 'left' --page-breaks-before '/'")
    File.delete("#{full_directory_path}.zip")
    FileUtils.rm_r(full_directory_path)
  end
end
